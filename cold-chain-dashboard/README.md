...
| 时间解析失败 | **已修复**：`skip_invalid_timestamp_rows=false` 时阻止导入并给出含行号、原始值、修正建议的清楚错误；`true` 时跳过该行并记录完整的 `skipped_rows.json` 日志（含行号、箱号、原始值、原因） |
| 温度字段无效 | 跳过该行，计入 skipped 计数 |
| 阈值配置错误 | 保存前校验数值有效性，无效值拒绝保存 |
| 重复导入同一批数据 | **已修复**：双层判断机制 — 完全重复（同一数据+同一配置）拒绝；重分析（同一数据+不同配置）允许并智能更新 |
| 必需列缺失 | 上传时立即报错，不生成任何事件 |
| 重启后导出一致性 | 事件状态、证据链、审计日志、跳过行日志完整保留，CSV/JSON 导出内容一致 |
